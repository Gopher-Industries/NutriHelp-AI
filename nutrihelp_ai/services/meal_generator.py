...
def pick_meal_under_calories(meals, remaining_calories):
    valid_meals = [m for m in meals if m.get("calories", 0) <= remaining_calories]
    if not valid_meals:
        return None
...
def plan(user, all_meals):
    ...
    if "elderly" in conditions_lower:
        soft_meals = [meal for meal in filtered if "soft_food" in meal.get("tags", [])]
        if len(soft_meals) > 0:
            filtered = soft_meals
    ...
    # Pick breakfast
    plan["breakfast"] = pick_meal_under_calories(breakfasts, remaining_calories)
    if plan["breakfast"]:
    ...
    if plan["dinner"] is None:
        ...
        plan["dinner"] = pick_meal_under_calories(soft_dinners, remaining_calories)
